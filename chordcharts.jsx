...
import { ChevronUp, ChevronDown, Save, Music4, Trash2, Copy, PlusCircle, Pilcrow, CaseUpper, Undo2, Tally3, Tally4, X, Scissors, ClipboardPaste, Redo2, FileDown, FolderOpen, Loader2 } from 'lucide-react';
...
export default function App() {
    ...
    useEffect(() => { if (auth) { onAuthStateChanged(auth, (user) => { if (user) setUserId(user.uid); else signInAnonymously(auth).catch(console.error); setIsAuthReady(true); }); } else { setIsAuthReady(true); } }, []);
    ...
    const handleSaveChart = async () => { if (!userId || !db || isSaving) return; setIsSaving(true); try { const savedAt = new Date().toISOString(); const chartToSave = { ...chart, savedAt, sections: JSON.stringify(chart.sections) }; const chartRef = doc(db, `artifacts/${appId}/users/${userId}/charts`, chartToSave.id); await setDoc(chartRef, chartToSave); setChart(p => ({...p, savedAt}), true); alert("¡Partitura guardada con éxito!"); } catch(error) { console.error("Error saving chart: ", error); alert("Ha ocurrido un error al guardar la partitura."); } finally { setIsSaving(false); } };
    ...
    const handleExportPDF = async () => {
        if (isExporting || !pdfScriptsLoaded) { if (!isExporting) alert('Las librerías de exportación no se han cargado. Por favor, espere un momento.'); return; }
        setIsExporting(true);
        try {
            ...
            pdf.setFontSize(14);
            pdf.text(chart.artist, MARGIN, y);
            y += 5;
            pdf.setFontSize(10);
            pdf.setTextColor(100);
            pdf.text(`Tonalidad: ${chart.key} ${chart.mode === 'major' ? 'Mayor' : 'Menor'}`, MARGIN, y);
            y += 15;

            const CHORD_FONT_SIZE = 12;
            const SECTION_FONT_SIZE = 14;
            const LINE_HEIGHT = 16; 
            const SECTION_SPACING = 12;
            const CONTENT_WIDTH = pdf.internal.pageSize.getWidth() - MARGIN * 2;
            const MEASURE_WIDTH = CONTENT_WIDTH / 4;
            ...
            for (const section of chart.sections) {
                ...
                pdf.setFont('helvetica', 'bold');
                pdf.setFontSize(CHORD_FONT_SIZE);
                    ...
                    lineMeasures.forEach((measure, j) => {
                        ...
                        if (j === 0) {
                           if (measure.startRepeat) {
                                ...
                                pdf.setLineWidth(i === 0 ? 1.2 : 0.2);
                                pdf.line(startBarX, measureYPos, startBarX, y);
                                if (i === 0) pdf.line(startBarX - 0.5, measureYPos, startBarX-0.5, y);
                            }
                        }
                        ...
                        if (measure.endRepeat) {
                            ...
                            if (repeatText) { pdf.setFontSize(10); pdf.text(repeatText, endBarX - 6, y - LINE_HEIGHT * 0.75); pdf.setFontSize(CHORD_FONT_SIZE); }
                             ...
                             pdf.line(endBarX, measureYPos, endBarX, y);
                        }
                        x += MEASURE_WIDTH;
                    });
                    y += LINE_HEIGHT;
        ...
        } catch (error) { console.error("PDF Export Error:", error); alert("Ocurrió un error al generar el PDF."); } 
    ...
    const ChordChart = () => ( <main className="pt-32 pb-64 px-2 md:px-4 bg-gray-100 text-gray-800 min-h-screen" onClick={clearSelection}> <div className="max-w-4xl mx-auto space-y-4 relative"> <ContextualMenu /> <div id="pdf-container" className="bg-white p-4 sm:p-6 lg:p-8 rounded-lg shadow-sm border"> <div className="flex justify-between items-end border-b-2 pb-2 mb-4"> <div><h1 className="text-3xl font-bold text-gray-800 font-sans">{chart.title}</h1><p className="text-lg text-gray-600 font-sans">{chart.artist}</p></div> <div className="text-right"><p className="text-md text-gray-500">Tonalidad: <span className="font-bold text-gray-700">{chart.key} {chart.mode === 'major' ? 'Mayor' : 'Menor'}</span></p></div> </div> {chart.sections.map((section, sectionIndex) => ( <div key={section.id} draggable onDragStart={(e) => handleDragStart(e, sectionIndex)} onDragOver={(e) => handleDragOver(e, sectionIndex)} onDragEnd={handleDragEnd} className={`p-3 rounded-lg transition-all border-2 ${draggedItem?.id === section.id ? 'border-blue-400' : 'border-transparent'} hover:border-gray-300 cursor-move ${selection.activeSectionId === section.id && !selection.start ? 'bg-blue-50 ring-2 ring-blue-300' : ''}`} onClick={(e)=>{e.stopPropagation(); setActiveSection(section.id);}}> <div className="flex justify-between items-center mb-3"> {editingSection?.id === section.id ? (<input type="text" value={editingSection.name} onChange={(e) => setEditingSection({...editingSection, name: e.target.value})} onBlur={() => handleRenameSection(section.id, editingSection.name)} onKeyDown={(e) => {if(e.key === 'Enter') handleRenameSection(section.id, editingSection.name);}} list="section-names" className="text-xl font-bold text-blue-700 bg-blue-50 px-3 py-1 rounded-md border-blue-400 border" autoFocus onClick={e => e.stopPropagation()}/>) : (<h2 onClick={(e) => { e.stopPropagation(); setEditingSection({id: section.id, name: section.name}) }} className="text-xl font-bold text-blue-700 bg-gray-100 px-3 py-1 rounded-md inline-block cursor-pointer hover:bg-gray-200">{section.name}</h2>)} <datalist id="section-names"><option value="Intro"/><option value="Verse"/><option value="Chorus"/><option value="Bridge"/><option value="Solo"/><option value="Outro"/><option value="Interlude"/></datalist> <div className="flex items-center gap-2 section-buttons"> <button onClick={(e) => {e.stopPropagation(); handleAddSection(sectionIndex)}} title="Añadir sección" className="p-1 text-gray-400 hover:text-blue-600"><PlusCircle size={18}/></button> <button onClick={(e) => {e.stopPropagation(); handleDuplicateSection(sectionIndex)}} title="Duplicar sección" className="p-1 text-gray-400 hover:text-blue-600"><Copy size={18}/></button> <button onClick={(e) => {e.stopPropagation(); handleDeleteSection(section.id)}} title="Eliminar sección" className="p-1 text-gray-400 hover:text-red-600"><Trash2 size={18}/></button> </div> </div> <div className="space-y-1 font-mono text-lg"> {Array.from({ length: Math.ceil(section.measures.length / 4) || (section.measures.length > 0 ? 1 : 0) }).map((_, lineIndex) => { if (lineIndex > 0 && section.measures.length <= lineIndex * 4) return null; return ( <div key={`line-${section.id}-${lineIndex}`} className="flex items-center w-full"> <div className="flex flex-1"> {Array.from({ length: 4 }).map((__, measureIndexInLine) => { const globalMeasureIndex = lineIndex * 4 + measureIndexInLine; const measure = section.measures[globalMeasureIndex]; if (!measure) return <div key={`empty-${globalMeasureIndex}`} className="h-10 flex-1"></div>; const isSelected = selection.start && selection.start.sectionId === section.id && globalMeasureIndex >= Math.min(selection.start.measureIndex, selection.end?.measureIndex ?? selection.start.measureIndex) && globalMeasureIndex <= Math.max(selection.start.measureIndex, selection.end?.measureIndex ?? selection.start.measureIndex); return ( <React.Fragment key={`frag-${globalMeasureIndex}`}> <div className={`h-10 flex items-center justify-center cursor-pointer rounded-sm flex-1 ${isSelected ? 'bg-green-100 ring-2 ring-green-500' : 'hover:bg-gray-100'}`} onClick={measure ? (e) => handleMeasureClick(e, section.id, globalMeasureIndex) : undefined}> {measure && measure.chords.length > 1 ? ( <div className="flex justify-around w-full"> <span>{measure.chords[0]}</span> <span>{measure.chords[1]}</span> </div> ) : ( <span className="whitespace-nowrap overflow-hidden text-ellipsis px-1">{measure ? measure.chords.join(' ') : '\u00A0'}</span> )} </div> {measureIndexInLine < 3 && <span className="text-gray-500 font-bold mx-1">|</span>} </React.Fragment> ); })} </div> <span className="text-gray-500 font-bold w-6 text-center">{ (lineIndex + 1) * 4 >= section.measures.length && section.measures.length > 0 ? '||' : '|' }</span> </div> ); })} </div> </div> ))} </div> </div> </main> );
